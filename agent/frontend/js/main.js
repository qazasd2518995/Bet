...
const app = createApp({
    data() {
        return {
            ...
            modalSystemReady: false, // 模態框系統是否準備就緒
            newMember: {
                username: '',
                password: '',
                confirmPassword: '',
    ...
    async mounted() {
        ...
        this.$nextTick(() => {
            ...
            // 延遲啟用模態框系統，確保所有組件都已初始化
            setTimeout(() => {
                this.modalSystemReady = true;
                console.log('🔓 模態框系統已啟用');
            }, 1000); // 延遲1秒確保一切就緒
        });
    },
    ...
    methods: {
        ...
        async loadRebateRecords() {
            ...
            try {
                ...
                if (!response.ok) {
                    console.error('載入退水記錄失敗:', response.status);
                    this.rebateRecords = [];
                    return;
                }
                
                const data = await response.json();
                console.log('退水記錄API回應:', data);
                ...
                if (data.success) {
                    ...
                    console.log('退水記錄載入成功:', this.rebateRecords.length, '筆，總金額:', this.totalRebateAmount);
                    this.showMessage(`載入 ${this.rebateRecords.length} 筆退水記錄`, 'success');
                } else {
                    console.error('載入退水記錄失敗:', data.message);
                    this.showMessage(`載入退水記錄失敗: ${data.message}`, 'error');
                    this.rebateRecords = [];
                }
            } catch (error) {
                console.error('載入退水記錄時發生錯誤:', error);
                this.showMessage('載入退水記錄時發生錯誤', 'error');
                this.rebateRecords = [];
            } finally {
                this.loading = false;
            }
        ...
        filterRebateRecords() {
            // 觸發computed屬性重新計算
            console.log('篩選退水記錄，條件:', this.rebateFilters);
