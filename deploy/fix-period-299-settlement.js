// fix-period-299-settlement.js - 修复期号299的结算错误
import db from './db/config.js';

async function fixPeriod299Settlement() {
    try {
        console.log('🔧 修复期号299的结算错误...\n');
        ...
        await db.tx(async t => {
            ...
            for (const bet of shouldWinBets) {
                ...
                console.log(`✅ 已修正投注ID ${bet.id}: ${bet.bet_type} ${bet.bet_value} (开出${bet.position_value}) -> 中奖 $${winAmount}`);
            }
            ...
            `, [
                ...
                newBalance, 
                `期号299结算错误补偿 - 修正${shouldWinBets.length}笔中奖投注`
            ]);
